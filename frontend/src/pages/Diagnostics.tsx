...
export function DiagnosticsPage() {
  ...
  const copyAllPrompts = async () => {
    ...
    const settingsHeader = [
      ...
      `AGENT_CAUTION_SIZE_MULT=${s?.agent_caution_size_mult ?? 'n/a'}`,
      `AGENT_CAUTION_MIN_RR=${s?.agent_caution_min_rr ?? 'n/a'}`,
      `AGENT_CAUTION_REQUIRE_CORROBORATION=${s?.agent_caution_require_corroboration ?? 'n/a'}`,
      `AGENT_PLAN_BACKFILL_ENABLED=${s?.agent_plan_backfill_enabled ?? 'n/a'}`,
      `AGENT_PLAN_BACKFILL_STOP_PCT=${s?.agent_plan_backfill_stop_pct ?? 'n/a'}`,
      `AGENT_PLAN_BACKFILL_TARGET_PCT=${s?.agent_plan_backfill_target_pct ?? 'n/a'}`,
      `AGENT_INVALIDATION_EXITS_ENABLED=${s?.agent_invalidation_exits_enabled ?? 'n/a'}`,
      `AGENT_INVALIDATION_SMA_PERIOD=${s?.agent_invalidation_sma_period ?? 'n/a'}`,
      `AGENT_INVALIDATION_CONSEC_CLOSES=${s?.agent_invalidation_consec_closes ?? 'n/a'}`,
      `AGENT_INVALIDATION_FIRST_CLOSE_ON_CONFIRMED=${s?.agent_invalidation_first_close_on_confirmed ?? 'n/a'}`,
      `SWING_TIME_STOP_MIN_PROGRESS_PCT=${s?.swing_time_stop_min_progress_pct ?? 'n/a'}`,
      `AGENT_WATCHLIST_AUTOADD_MODE=${s?.agent_watchlist_autoadd_mode ?? 'n/a'}`,
      `AGENT_WATCHLIST_COOLDOWN_HOURS=${s?.agent_watchlist_cooldown_hours ?? 'n/a'}`,
      `AGENT_AUTO_EXECUTE_PAPER=${s?.agent_auto_execute_paper ?? 'n/a'}`,
      `AGENT_DISABLE_SAME_RUN_REDEPLOY=${s?.agent_disable_same_run_redeploy ?? 'n/a'}`,
      `AGENT_USE_INTRADAY_CONFIRMATION=${s?.agent_use_intraday_confirmation ?? 'n/a'}`,
      `AGENT_INTRADAY_LOOKBACK_MINUTES=${s?.agent_intraday_lookback_minutes ?? 'n/a'}`,
      '',
    ]
    const chunks: string[] = [
      ...settingsHeader,
