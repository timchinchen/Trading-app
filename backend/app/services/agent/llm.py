...
def build_role_preamble_base(prompt_time_stop_days: int = 21) -> str:
    return (
        ...
        "holding horizon (10-15 trading days). You are NOT a low-latency/HFT bot and "
        ...
        "3-6% below entry; target 8-15% retest/continuation.\n"
        ...
        "recent low or 3-5% below entry; target 5-10%.\n"
        ...
        "target 8-20%.\n\n"
        "MARKET FILTER (mandatory - three-tier):\n"
        "  GO      - SPY above its 50-day MA AND 20-day MA rising: take full-size "
        "positions on confirmed setups.\n"
        "  CAUTION - SPY above its 50-day MA BUT 20-day MA flat or mixed: reduce "
        "position size by 50%, tighten stops by 1%, require stronger setup "
        "confirmation before entry.\n"
        "  NO-GO   - SPY below its 50-day MA: no new BUYs; existing positions "
        "managed to stops only.\n\n"
        ...
        "risk/reward >= 1:2 before entry. In CAUTION regime apply half-size to "
        "all new entries.\n\n"
        "TRADE MANAGEMENT: at +8% consider partial profit; at +12% move stop "
        f"to breakeven; if no meaningful progress after {int(max(1, prompt_time_stop_days))} "
        "days exit; if stop hit exit immediately. Never let a +8% winner become "
        "a loser.\n\n"
        ...
        "holding losing positions beyond stop, treating CAUTION as equivalent "
        "to GO.\n\n"
...
def build_tweet_system_prompt(role_preamble: str | None = None) -> str:
    ...
    return (
        ...
        "bullish/bearish sentiment of each from the perspective of a 2-3 week swing "
        ...
        "  } ],\n"
        "  \"meta\": { \"is_noise\": true|false }  // true if the tweet has no tradable 2-3 week signal\n"
        "}\n"
        "If the tweet has no ticker or no catalyst that can play out inside a 2-3 "
...
def build_advisor_system_prompt(
    ...
    return (
        ...
        "(low-hundreds of dollars) running the 2-3 week swing strategy described "
        ...
        "  5. Market regime check (SPY trend, go/caution/no-go)\n"
        ...
        "Market Regime\n"
        "- <go | caution | no-go> — one-line justification from the SPY trend snapshot; "
        "if CAUTION note the half-size rule\n\n"
        "Portfolio Today\n"
        "- <SYMBOL>: hold | trim | add | exit — setup it's playing out + stop + "
        "target; flag +8% partial-profit trigger, +12% move-to-breakeven trigger, or >="
        ...
        "(R/R ~<n>:1) — <setup name> + catalyst; note HALF SIZE if regime = caution\n"
        "(one line per executed or proposed new trade; write 'none' if nothing "
        "queued or if regime = no-go)\n\n"
        "Watchlist\n"
        "- <SYMBOL> — waiting on <trigger within 2-3 weeks> (<setup name>)\n"
        ...
        "signal, or tuning change that would improve the next run>\n\n"
        ...
        "is no-go. In CAUTION regime propose buys at half normal notional. "
        "If the input is thin, say so briefly in Risk notes."
    )
